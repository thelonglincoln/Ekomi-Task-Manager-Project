...
class EkomiTaskManagerSupabaseUser extends BaseAuthUser {
  EkomiTaskManagerSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> ekomiTaskManagerSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = EkomiTaskManagerSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
